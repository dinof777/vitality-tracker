...
| 10 | [MAJOR] [since 2026-08-29] **The live branded gym page overflows horizontally on a phone.** Found while capturing the deployed `/g/ironforge` at a 390×844 viewport for the title fix (finding #6) — Chrome reports the page content as **500px wide in a 390px viewport**, so roughly a fifth of every row is cut off the right edge: the "Refresh all" button, the right end of the SEND TO SYNCROFIT and BUILD A WORKOUT buttons, and the SAVE button next to the workout-name field are all partially or wholly off-screen. Reproduce with `cd /Users/dinoflora/dev/vitality-tracker && node capture.mjs https://www.liveelevated.fit/g/ironforge /tmp/m.png 390 844` — it prints the overflow warning itself. This is a phone-first product and this is the white-label customer's own page, so it is the worst place for it. Not investigated (out of scope for that task); the offending element has not been identified. Owner: **Kevin** (build) with **Iris** if it turns out to be a layout/render issue. |
