...
export async function GET(): Promise<Response> {
  ...
  const lines: string[] = [
    '# Vitality',
    '',
    `> Vitality is a mobile-first workout app — a ${moves}-move illustrated library across ${equip} equipment types, with time-budgeted workout generation, routines, weekly planning, and a daily mobility habit. It's also "Vitality Pro," a white-label platform that lets gyms and personal trainers run a branded copy of the app for their clients and push workouts to the SyncroFit interval timer.`,
    '',
    ...
    `- [Move library](${u('/exercises')}): All ${moves} moves with illustrations, muscle group, difficulty tier, and coaching cues; searchable, grouped by equipment.`,
    ...
    `- [Trainer dashboard](${u('/dashboard')}): Manage your gym — branding, custom moves, and equipment.`,
    `- [Branded gym app (example)](${u('/g/vitality')}): A gym's public, themed home screen.`,
    `- [A gym's move library](${u('/g/vitality/exercises')}): The global library plus the gym's own custom moves and local renames.`,
    ...
    '- Custom moves and per-gym renames (call a move whatever your gym calls it).',
