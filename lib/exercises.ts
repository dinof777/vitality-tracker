...
// Move library (291) — seeded into the Neon `exercises` table (same UUIDs).
...
export const SAMPLE_EXERCISES: Exercise[] = [
  ...
  // ===== Heavy lifts (21) — trainer's spec. Adds `bench` as its own equipment
  // (any lift where back/chest/hips are SUPPORTED by the bench, regardless of
  // barbell vs dumbbell load) plus a barbell/cable/leg-press/pull-up-bar
  // roster of heavier compound lifts. Rack pulls stay `barbell` — no
  // squat_rack value (rack access is already bundled into `barbell`, per
  // 0008's documented decision). image_url null → equipment-icon fallback
